#!/usr/bin/env bash
# Downloads the latest sing-box binary for the current Linux arch.
...
echo "Downloading sing-box v${VERSION} (${ARCH_TAG})..."
curl -fsSL "$URL" -o "$TMP/sing-box.tar.gz"
tar -xzf "$TMP/sing-box.tar.gz" -C "$TMP"
cp "$TMP/sing-box-${VERSION}-linux-${ARCH_TAG}/sing-box" "$DEST"
chmod +x "$DEST"
