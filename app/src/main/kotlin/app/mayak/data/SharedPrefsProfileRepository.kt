...
class SharedPrefsProfileRepository(context: Context) : ProfileRepository {
    private val prefs = context.getSharedPreferences("mayak_profiles", Context.MODE_PRIVATE)
    ...
    override fun observeSettings(): Flow<MayakSettings> = settings.asStateFlow()
    ...
    override suspend fun updateSettings(settings: MayakSettings) {
    ...
    override suspend fun currentSettings(): MayakSettings = settings.value
    ...
    private fun loadSettings(): MayakSettings {
        val raw = securePrefs.getString(KEY_SETTINGS) ?: return MayakSettings()
        val decoded = runCatching {
            json.decodeFromString<MayakSettings>(raw)
        }.getOrDefault(MayakSettings())
...
private class SecurePrefs(
    ...
    private companion object {
        const val KEY_ALIAS = "mayak_profiles_key"
