...
class TrafficMonitor(private val controllerPort: Int) {
        ...
        thread = Thread {
            ...
            while (running.get()) {
                val ok = runCatching { stream(onSample) }
                    .onFailure { System.err.println("[Beacon] traffic stream error: ${it.message}") }
                    .isSuccess
                if (!running.get()) break
                attempts++
                if (attempts > 40) {
                    System.err.println("[Beacon] traffic monitor giving up after $attempts attempts")
                    return@Thread
                }
                Thread.sleep(if (ok) 500 else 1500)
            }
        }.apply {
            name = "beacon-traffic-monitor"
            isDaemon = true
            start()
        }
    }
    ...
    private fun stream(onSample: (TrafficSample) -> Unit) {
        ...
        if (conn.responseCode != 200) {
            System.err.println("[Beacon] clash /traffic returned ${conn.responseCode}")
            conn.disconnect()
            return
        }
