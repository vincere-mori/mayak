...
enum class InboundMode(val title: String, val hint: String) {
    ...
    Tun(
        ...
        "Виртуальный сетевой адаптер перехватывает весь трафик компьютера — браузер, игры, мессенджеры, всё. Требует запуск от имени администратора, Маяк сам предложит перезапуститься."
    );
